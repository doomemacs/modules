...
* Description :unfold:
This module provides code completion, powered by [[pkg:corfu]].

It is recommended to enable either this or [[module::completion company]] in
...
- +orderless ::
  Pull in [[pkg:orderless]] if necessary and apply multi-component
  completion (still needed if [[module::completion vertico]] is active).
- +dabbrev ::
  Enable and configure [[pkg:dabbrev]] as a close-to-universal CAPF
  fallback.

** Packages
- [[package:corfu]]
- [[package:cape]]
- [[package:nerd-icons-corfu]] if [[module::completion corfu +icons]]
- [[package:orderless]] if [[module::completion corfu +orderless]]
- [[package:corfu-terminal]] if [[module::os tty]]
- [[package:yasnippet-capf]] if [[module::editor snippets]]
...
If the [[module::completion corfu +orderless]] flag is enabled, users can
...
Vertico for instance, one could use this to export with [[pkg:embark]] via
...
[[pkg:cape]]'s adapter to enable this.
