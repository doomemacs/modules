...
- +icons ::
  ...
  [[pkg:nerd-icons]] or [[pkg:treemacs]] iconsets.
...
- [[package:helm-flx]]* if [[module:+fuzzy]]
- [[package:helm-icons]]* if [[module:+icons]]
- [[package:helm-org]]* if [[module::lang org]]
- [[package:helm-posframe]]* if [[module:+childframe]]
...
* TODO Usage
Much like [[pkg:ivy]] and [[pkg:vertico]], [[pkg:helm]] is a /large/ framework. Covering
...
Changes to the resulting [[pkg:wgrep]] buffer (opened by [[kbd:][C-c C-e]]) can be
committed with [[kbd:][C-c C-c]] and aborted with [[kbd:][C-c C-k]] (alternatively [[kbd:][ZZ]] and [[kbd:][ZQ]], for
evil users).

** In-buffer searching
The [[pkg:swiper]] package provides an interactive buffer search powered by helm. It can
be invoked with:
...
A [[pkg:wgrep]] buffer can be opened from swiper with [[kbd:][C-c C-e]].
...
[[pkg:nerd-icons]] and [[pkg:treemacs]]. By default, and to
maintain consistency with other icons across Doom's modules,
[[pkg:nerd-icons]] is used. To change this:
#+begin_src emacs-lisp
;; add to $DOOMDIR/config.el
(with-eval-after-load 'helm
