...
workable results ripgrep produces, despite the error."
    :around #'counsel-rg
    (letf! (defadvice process-exit-status (:filter-return (code))
             (if (= code 2) 0 code))
      (apply fn args)))
...
(use-package! ivy-prescient
  ...
  (setq prescient-save-file (doom-profile-cache-dir t "prescient-save.el")))
...
;;;###package amx
(setq amx-save-file (doom-profile-cache-dir t "amx-items"))  ; used by `counsel-M-x'
