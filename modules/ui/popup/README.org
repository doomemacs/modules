...
** Hacks
- [[package:help-mode]] has been advised to follow file links in the buffer you were in
  before entering the popup, rather than in a new window.
- [[package:wgrep]] buffers are advised to close themselves when aborting or committing
  changes.
- [[package:persp-mode]] is advised to restore popup windows when loading a session from
  file.
...
- [[package:neotree]] advises ~balance-windows~, which causes major slow-downs when paired
  with our ~balance-window~ advice, so we removes neotree's advice.
- [[package:org-mode]] is an ongoing (and huge) effort. It has a scorched-earth window
...
#+begin_quote tip
Popup rules end up in var:display-buffer-alist, which instructs
func:display-buffer calls on how to set up windows for buffers that meet certain
conditions. However, some plugins can avoid it entirely if they use
func:set-buffer or func:switch-to-buffer, which don't obey
var:display-buffer-alist.
#+end_quote

Multiple popup rules can be defined with [[fns:set-popup-rules!]]:
#+begin_src emacs-lisp
(set-popup-rules!
...
Omitted parameters in a [[fns:set-popup-rules!]] will use the defaults set in
var:+popup-defaults.

** Disabling hidden mode-line in popups
By default, the mode-line is hidden in popups. To disable this, you can either:

1. Change the default ~:modeline~ property in var:+popup-defaults:
   #+begin_src emacs-lisp
