...
[[minor:mac-auto-operator-composition-mode]] (on supported macOS systems) or composition
tables (harfbuzz on Emacs 28), falling back on [[minor:prettify-symbols-mode]] otherwise.
...
(plist-put +ligatures-extra-symbols :quote "“")
(plist-put +ligatures-extra-symbols :true "𝕋")
#+end_src

See [[var:+ligatures-extra-symbols]] for documentation and available properties.
...
want to use. [[fns:set-font-ligatures!]] is a thin wrapper around [[pkg:ligature]] to
control these.
...
This call will:
- Overwrite all preceding calls to [[fns:set-font-ligatures!]] for [[major:haskell-mode]] and
  [[major:clojure-mode]] specifically, but...
- Keep the inheritance to ligatures set for all modes, or parent modes like
  [[major:prog-mode]].
...
- Disable the [[module::ui unicode]] module. It overrides [[var:doom-symbol-font]]
