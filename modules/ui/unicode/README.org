...
This module relies on the [[github:rolandwalker/unicode-fonts][unicode-fonts]] package. It tries to setup the default
emacs fontset to cover as many unicode glyphs as possible by scanning all
available glyphs from all available fonts.

When this module is enabled:
- Emacs will prefer to use [[var:doom-symbol-font]] to display non-latin glyphs if
  it provides coverage for them.
...
- The cache will be regenerated every time Emacs is made aware of new fonts or
  you change the font configuration e.g. by modifying [[var:doom-symbol-font]].
...
** TODO Emacs daemon mode ([[github:doomemacs/core/issues/3328][#3328]])
This module may fail to setup fonts when emacs is run in a daemon session.

** Common error: ~void-variable \.\.\.~ ([[github:doomemacs/core/issues/7164][#7164]])
...
[[github:rolandwalker/unicode-fonts/issues/36][rolandwalker/unicode-fonts#36]]. A workaround is to delete your unicode-fonts
cache and restart Emacs:
#+begin_src bash
