...
** Packages
- [[package:evil-magit]] if [[module::editor evil +everywhere]]
- [[package:forge]] if [[module:+forge]]
- [[package:code-review]] if [[module:+forge]]
- [[package:magit]]

** Hacks
- [[package:magit]] has been modified to recognize =$XDG_CACHE_HOME/git/credential/socket=.
- [[package:magit]] has been modified to invalidate the projectile cache when you check out
  a new branch or commit.
- [[package:magit]] has been modified to revert repo buffers (e.g. after changing branches)
  when you later switch to them, rather than all at once.
- [[package:forge]] was modified to defer compilation of emacsql-sqlite until you try to use
...
- [[module:+forge]] requires [[https://magit.vc/manual/forge/Token-Creation.html#Token-Creation][a Github API token]]
- [[package:code-review]] will also require a token setup (see [[https://github.com/wandersoncferreira/code-review#configuration][configuration]] for your particular forge)
