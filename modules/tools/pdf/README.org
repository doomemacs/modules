...
It uses [[pkg:pdf-tools]], which is a replacement for the built-in ~doc-view-mode~ for
...
Displaying PDF files is just one function of [[pkg:pdf-tools]]. See [[https://github.com/vedang/pdf-tools][its project website]]
...
- [[package:org-pdftools]] if [[module::lang org]]
...
[[pkg:pdf-tools]] plugin will (re)build this program for you, if you issue the ~M-x
pdf-tools-install~ command. See the next section for instructions on how to
build the =epdfinfo= program on Windows.

Build instructions for various OSes can be found [[https://github.com/vedang/pdf-tools?tab=readme-ov-file#installing-the-epdfinfo-server-from-package-managers][on the pdf-tools README]].

** Building epdfinfo on Guix
The [[https://packages.guix.gnu.org/packages/emacs-pdf-tools][emacs-pdf-tools]] package provides a prebuilt =epdfinfo=.
