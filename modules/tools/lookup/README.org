...
- +offline ::
  Install and prefer offline dictionary/thesaurus (with [[module:+dictionary]]).
- +yandex ::
  Add Yandex, Yandex Images, and Yandex Maps to the lookup backends for the
  [[cmd:+lookup/online]] command.

** Packages
- [[package:dumb-jump]]
- [[package:helm-xref]] if [[module::completion helm]]
- [[package:ivy-xref]] if [[module::completion ivy]]
- [[package:request]]
- if [[module:+docsets]]
  - [[package:dash-docs]]
  - [[package:counsel-dash]] if [[module::completion ivy]]
  - [[package:helm-dash]] if [[module::completion helm]]
...
- ~sqlite3~ for Dash docset support (if you have [[module:+docsets]] enabled)
- ~wordnet~ for offline dictionary and thesaurus support (if you have
  [[module:+dictionary]] and [[module:+offline]] enabled).
...
3. [[pkg:dumb-jump]] (a text search with aides to reduce false positives).
...
2. Any available [[pkg:xref]] backends.
3. An ordinary project-wide text search with ripgrep.
...
- ~:file FN~ ::
  ...
  by ~+lookup/file~.
- ~:xref-backend FN~ ::
  Defines an [[pkg:xref]] backend, which implicitly provides ~:definition~ and
...
#+begin_quote tip
~+lookup/in-docsets~ consults ~dash-docs-browser-func~ instead, which is already set
to ~#'eww~ by default.
#+end_quote
