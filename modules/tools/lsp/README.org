...
[[pkg:imenu]]/[[pkg:xref]] integration, jump-to-definition/references support, and more.

Depending on the presence of the =+eglot= flag, this module is powered by either
[[pkg:lsp-mode]] or [[pkg:eglot]]. A comprehensive list of LSP clients that are supported
...
** Module flags
- +booster ::
  *For Emacs 29 users only:* speeds up I/O between LSP clients and Emacs by
  converting JSON output from LSP servers into Elisp bytecode. Only works for
  Eglot, at the moment, and requires [[https://github.com/blahgeek/emacs-lsp-booster][emacs-lsp-booster]] (also needed on remote
  systems if editing over TRAMP).
- +eglot ::
...
- +peek ::
  Use ~lsp-ui-peek~ when looking up definitions and references with functionality
  from the [[module::tools lookup]] module.

** Packages
- if [[module:+eglot][+eglot]]
  - [[package:eglot]]
  - [[package:jsonrpc]]
  - [[package:consult-eglot]] if [[module::completion vertico][:completion vertico]]
  - [[package:flycheck-eglot]] if [[module::checkers syntax -flymake][:checkers syntax -flymake]]
  - [[package:eglot-booster]] if [[module:+booster]] and Emacs<30
- if [[module:-eglot]]
  - [[package:lsp-mode]]
  - [[package:lsp-ui]]
  - [[package:lsp-ivy]] if [[module::completion ivy]]
  - [[package:helm-lsp]] if [[module::completion helm]]
  - [[package:consult-lsp]] if [[module::completion vertico]]
...
3. Enable the [[module:+lsp]] flag on the [[module::lang]] modules you want to enable LSP support for.

Different languages will need different language servers, some of which [[pkg:lsp-mode]]
will prompt you to auto-install, but [[pkg:eglot]] will not.
...
Without the [[module:+eglot]] flag, and when [[module::completion ivy]], [[module::completion helm]] or
[[module::completion vertico]] is active, LSP is used to search a symbol indexed by the LSP
server:
| Keybind | Description                         |
|---------+-------------------------------------|
...
- [[package:lsp-mode]] tends to be more featureful, beginner-friendly (e.g. offers to
...
- [[package:eglot]] has fewer bells and whistles: it relies on built-in Emacs functionality
...
in/out the [[module:+lsp]] or [[module:+eglot]] flag when [[id:01cffea4-3329-45e2-a892-95a384ab2338][enabling this module]].
...
Make sure that you have enabled the [[module:+lsp]] flag on the appropriate module(s) (in
