...
documentation of their respective [[module::lang]] module for instructions.
...
- ~M-x +eval/buffer-or-region-in-repl~ ([[kbd:][SPC c s]]) while a selection (and REPL) is
  active
...
#+begin_src emacs-lisp
(set-eval-handler! 'emacs-lisp-mode #'+emacs-lisp-eval-fn)
#+end_src
