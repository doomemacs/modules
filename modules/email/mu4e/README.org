...
- Tidied mu4e headers view, with flags from [[pkg:nerd-icons]].
...
It uses ~mu4e~ to read my email, but depends on ~offlineimap~ (to sync my email via
IMAP) and ~mu~ (to index my mail into a format ~mu4e~ can understand).
#+end_quote
...
- +org ::
  Use [[pkg:org-msg]] for composing email in Org, then sending a multipart text (ASCII
  export) and HTML message.

** Packages
- [[package:mu4e-compat]]
- [[package:org-msg]] if [[module:+org]]
...
(~mu4e--get-mail-password-regexp~ if =mu= *>=1.8*) such that [[pkg:mu4e]] will recognize
...
*** Gmail
With the [[module:+gmail]] flag, integrations are applied which account for the different
behaviour of Gmail.
...
the [[module::tools pass +auth]] module as an auth source. The corresponding
...
workaround [[elisp:(info "(mu4e) Writing messages")][in mu4e's manual]] that uses the [[pkg:async]] library. To use it,
...
If you're using [[module::tools pass +auth]], you also need to ensure that the child
...
** OrgMsg
With the [[module:+org]] flag, [[pkg:org-msg]] is installed, and ~org-msg-mode~ is enabled before
...
compose or reply command ([[kbd:][SPC u]] with [[pkg:evil]], [[kbd:][C-u]] otherwise).
