...
** Packages
- [[package:clipetty]] if [[module:+osc]]
- [[package:evil-terminal-cursor-changer]] if [[module::editor evil]]
- [[package:kkp]]
- [[package:xclip]] unless [[module:+osc]]
...
- For system clipboard integration:
  ...
  - (If [[module:+osc]] is enabled) A terminal that supports OSC 52 escape codes, such as:
...
** Can't abort some processes with [[kbd:][C-g]]
This module installs kkp.el, which adds Kitty Keyboard Protocol support to TTY
Emacs, which, unfortunately, also captures [[kbd:][C-g]] keypresses, rendering it
ineffective at aborting

This is better explained [[https://github.com/benotn/kkp#c-g-and-blocking-subprocesses][in kkp's README]], along with  workarounds. Doom has
employed a workaround for envrc, and will cover its own modules, but you must
take care of any functionality beyond that.
