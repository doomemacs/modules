...
[[pkg:apheleia]] will fail silently.
...
If you have a buffer open with [[pkg:lsp-mode]] or [[pkg:eglot]]
enabled, and the running server supports =textDocument/formatting= or
=textDocument/rangeFormatting=, it can be used instead of
[[pkg:apheleia]]'s (or Doom's) default formatters by enabling this module
...
* Troubleshooting
There are a few fail-safes [[pkg:apheleia]] has to prevent accidental code
