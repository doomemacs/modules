...
- [[package:evil]]
- [[package:evil-args]]
- [[package:evil-collection]] if [[module:+everywhere]]
...
| vim-unimpaired        | (provided by Doom)             | [[https://github.com/doomemacs/modules/blob/master/modules/editor/evil/config.el#L413-L460][see the list]]                   |

This module has also ported vim-unimpaired keybinds to Emacs.

In other modules:
- The [[module::ui neotree]] & [[module::ui treemacs]] modules provide a =NERDTree= equivalent.
- The [[module::editor multiple-cursors]] module contains functionality equal to the
  following vim plugins:
  - [[package:evil-multiedit]] => [[github:hlissner/vim-multiedit][vim-multiedit]]
  - [[package:evil-mc]] => [[https://github.com/terryma/vim-multiple-cursors][vim-multiple-cursors]]
...
| ~:a~                     | Open sibling file (e.g. .cpp -> .hpp, .scss -> .css, etc)                          |
| ~:al[ign][!]/REGEXP~     | Align text to the first match of REGEXP. If BANG, align all matches on each line   |
| ~:cap[ture]~             | Invoke ~org-capture~                                                                 |
| ~:cd DIRECTORY~          | Change ~default-directory~ to =DIRECTORY=                                              |
...
| ~:kill~                  | Kill the current buffer                                                            |
| ~:killa[ll][!]~          | Kill all buffers (if BANG, affect buffer across workspaces)                        |
| ~:killb[uried]~          | Kill all buried buffers                                                            |
| ~:killm[atch][!] REGEXP~ | Kill buffers whose name matches REGEXP (if BANG, affect buffers across workspaces) |
| ~:killo[ther]~           | Kill all other buffers besides the selected one                                    |
| ~:l[ast]~                | Reopen last popup                                                                  |
| ~:ldiff~                 | Ediff two regions (must be used twice)                                             |
| ~:lo[okup] QUERY~        | Look up QUERY on an online search engine                                           |
| ~:mc[!]/REGEXP~          | Invoke multiple cursors on all matches for REGEXP (if BANG, REGEXP is literal)     |
| ~:msg~                   | Open \*Messages\* buffer                                                           |
...
| ~:pop[up]~               | Convert the selected window to a popup window                                      |
| ~:pwd~                   | Display the current ~default-directory~ in echo area                                 |
...
| ~:sh[ell] CMD~           | Execute a command in the last opened eshell buffer (or make one)                   |
| ~:sclear~                | ([[module::ui workspaces]]) Kill all workspaces                                   |
| ~:sl[oad]~               | ([[module::ui workspaces]]) Load the last saved session                           |
| ~:ss[ave]~               | ([[module::ui workspaces]]) Save the current session                              |
| ~:tabc[lose]~            | ([[module::ui workspaces]]) Close the current workspace                           |
| ~:tabclear~              | ([[module::ui workspaces]]) Kill all buffers in current workspace                 |
| ~:tabf[irst]~            | ([[module::ui workspaces]]) Switch to the first workspace                         |
| ~:tabl[ast]~             | ([[module::ui workspaces]]) Switch to the last workspace                          |
| ~:tabload [NAME]~        | ([[module::ui workspaces]]) Load a workspace from file                            |
| ~:tabn[ew][!]~ [NAME]    | ([[module::ui workspaces]]) Create a new workspace                                |
| ~:tabnext~               | ([[module::ui workspaces]]) Switch to next workspace                              |
| ~:tabprev~               | ([[module::ui workspaces]]) Switch to previous workspace                          |
| ~:tabr[ename]~ NAME      | ([[module::ui workspaces]]) Rename the current workspace to NAME                  |
| ~:tabs~                  | ([[module::ui workspaces]]) Display all workspaces in the echo area               |
| ~:tabsave [NAME]~        | ([[module::ui workspaces]]) Save the selected workspace to file                   |
...
(setopt evil-want-C-u-scroll nil
        evil-want-C-u-delete nil)
#+end_src

** The [[kbd:][s]]/[[kbd:][S]] keys behave differently from Vim
Doom replaces the [[kbd:][s]] and [[kbd:][S]] keys with the [[pkg:evil-snipe]] package (a port of
...
2. Or disable [[pkg:evil-snipe]] completely with ~(package! evil-snipe :disable t)~
...
[[doom-contrib-faq:][Ask a question?]]
...
#+begin_quote warning
Ignore ~doom-leader-key~ and ~doom-localleader-key~, they don't apply to non-evil
sessions.
#+end_quote

Evil-specific configuration and keybindings (defined with ~map!~) will be
ignored without [[module::editor evil]] present (and omitted when byte-compiling).
