...
** Module flags
- +lsp ::
  Enable LSP support for ~c-mode~, ~c++-mode~, and ~objc-mode~. Requires [[module::tools lsp]]
...
- [[package:modern-cpp-font-lock]] unless [[module:+tree-sitter]]
- [[package:opencl-mode]]
- if [[module:+lsp]]
  - [[package:ccls]] if [[module::tools lsp -eglot]]
...
- If [[module:+lsp]] is enabled, you need one of *clangd v9+* or *ccls*.
...
** =:editor format=
The formatter used is =clang-format= which should be installed alongside =clang=.

For more info, see [[module::editor format]].
...
When using [[module:+lsp]] and [[module::tools lsp +eglot]], [[pkg:lsp-mode]] is replaced with [[pkg:eglot]], and an
...
[[pkg:lsp-mode]] by default, [[pkg:eglot]] if you have [[module::tools lsp +eglot]] active in
