...
- A supported syntax checker (some require Emacs packages that this module does
  ...
  | pyflakes   | needs [[doom-package:flycheck-pyflakes][package]] | built-in [fn:1]     |
  | ruff       | built-in                | built-in [fn:1]     |
  | flake8     | built-in                | built-in [fn:1]     |
  | pyright    | built-in                | no              |
  | py_compile | built-in                | no              |
  | mypy       | built-in                | no              |
  | cython     | needs [[doom-module:+cython]]           | no              |
...
- The [[doom-module::editor format]] module uses [[https://github.com/psf/black][Black]] for
