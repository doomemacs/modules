...
- Optional Intel Fortran support via the [[module:+intel]] flag.
...
now... Cheers Dad, hope this helps. --[[github:fosskers]]
#+end_quote
...
** Module flags
- +lsp ::
  Enable LSP support for ~fortran-mode~. Requires [[module::tools lsp]] and a langserver
  (fortls).
- +intel ::
...
Activating the [[module:+intel]] flag won't automatically install Intel Fortan for you.
...
** Auto-formatting
When [[module::editor format]] is enabled and [[https://github.com/fortran-lang/fprettify][fprettify]] is installed, buffers can be
formatted with [[fn:apheleia-format-buffer]].

Enable [[module::editor format +onsave]] to format the buffer on save. See below for advanced configuration.
