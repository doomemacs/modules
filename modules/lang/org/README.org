...
- Drag-and-drop support for images (with inline preview) and media files (drops
  a file icon and a short link) (requires [[module:+dragndrop]] flag).
...
- +crypt ::
  Enable [[pkg:org-crypt]] integraiton for encryption and decryption on org
  files.
- +gnuplot ::
  ...
  default).
- +journal ::
  Enable [[pkg:org-journal]] integration.
- +jupyter ::
  Enable Jupyter integration for babel.
- +noter ::
  Enable [[pkg:org-noter]] integration. Keeps notes in sync with a
  document. Requires [[module::tools pdf]], [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Document-View.html][DocView]], or [[https://github.com/wasamasa/nov.el][nov.el]] to be enabled.
- +pandoc ::
...
- Adds support for a ~:sync~ parameter for org src blocks. This overrides
  ~:async~.
- Gracefully degrades ~:async~ babel blocks to ~:sync~ when [[pkg:ob-async]] would cause
  errors or issues (such as with a ~:session~ parameter, which [[pkg:ob-async]] does not
...
- If [[module::ui workspaces]] is enabled, persp-mode won't register org agenda buffers
...
- To render GNUPlot images (with [[module:+gnuplot]] flag) the ~gnuplot~ program is needed.
- To execute babel code blocks, you need whatever dependencies those languages
  need. It is recommended you enable the associated [[module::lang]] module and ensure its
  dependencies are met, e.g. install the ~ruby~ executable for ruby support. To
  use ~jupyter kernels~ you need the [[module:+jupyter]] flag, the associated kernel as
  well as the ~jupyter~ program.
- [[package:org-roam]] (with the [[module:+roam]] flag) requires Emacs to be
...
For [[module:+jupyter]], install =jupyterlab= or =jupyter-notebook= ([[https://wiki.archlinux.org/title/Jupyter][source]]).
...
~org-directory~ must be set /before/ [[pkg:org]] has loaded:
#+begin_src emacs-lisp
