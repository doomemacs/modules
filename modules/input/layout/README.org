...
- Changing navigation keys to [[kbd:c]], [[kbd:t]], [[kbd:s]], and [[kbd:r]]
  ...
  + old [[kbd:s]] is mapped to [[kbd:k]] (i.e. staging in the magit status buffer is done with
    [[kbd:k]])
  + See [[*Configuration][Configuration]] to see where old [[kbd:c]] and [[kbd:r]] functions are remapped
...
*** Easymotion
If you use [[pkg:evil-easymotion]], then all the bindings that were on [[kbd:gs]] have been
moved to [[kbd:gé]].
...
*** Org-mode
[[pkg:evil-org]] allows to define ~evil-org-movement-bindings~ to automatically map
...
*** Bépo
- In [[pkg:eshell]], the key [[kbd:c]] is still bound to ~evil-collection-eshell-evil-change~ in
  normal mode.
