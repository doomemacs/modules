...
This module requires the [[https://dejavu-fonts.github.io/][DejaVu Sans]] font be installed and present on your
system (unless you've changed ~+bidi-hebrew-font~ and ~+bidi-arabic-font~ to a
different font).

* Usage
This module provides [[minor:+bidi-mode]], a minor mode that improves the display of RTL
...
(Arabic, Persian, Urdu, etc.) in [[var:+bidi-hebrew-font]] and
[[var:+bidi-arabic-font]]. See [[Fonts]] for more information. If you use an RTL
language that isn't covered by these characters, open an issue requesting
support for it.

To resize fonts, don't include a ~:size~ parameter in [[var:+bidi-hebrew-font]] or
[[var:+bidi-arabic-font]], set [[var:+bidi-hebrew-font-scale]] or
var:+bidi-arabic-font-scale instead, or add an entry to
[[var:face-font-rescale-alist]] for your font. E.g.
...
#+begin_quote warning
Do not do this if you are using ~+bidi-global-mode~, it will mess up all of the
buffers in Emacs that use English, including things like the =M-x= buffer.
#+end_quote
...
Pashto, you may want to change [[var:+bidi-arabic-font]] to one specific to your
...
[[fns:set-fontset-font]] for more details.
...
If you only use [[minor:+bidi-mode]] in specific buffers, you might want to automatically
change the dictionary language there. For example:
#+begin_src emacs-lisp
